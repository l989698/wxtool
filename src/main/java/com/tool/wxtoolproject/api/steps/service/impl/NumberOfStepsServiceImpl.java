package com.tool.wxtoolproject.api.steps.service.impl;
...
@Service
public class NumberOfStepsServiceImpl implements NumberOfStepsService {

    @Override
    public Result modifyWeChatSteps(ModifyWeChatStepsParam param) {
        String sport = this.sport(param);
     ...
     * @return
     */
    public static String sport(ModifyWeChatStepsParam param) {
        String[] login = login(param.getUsername(), param.getPassword());
        if (login == null) {
            return null;
        }
        String t = getTime();
        String dataJson = dataJson(DateUtil.today(), param.getSteps());
